      ...
      <Card title={data.name}>
        <Row gutter={[32, 32]}>
          {data.indicators.map((indicator, index) => {
            const isLastOdd =
              data.indicators.length % 2 !== 0 && index === data.indicators.length - 1;
            return (
              <Col key={indicator.id} xs={24} sm={24} xl={isLastOdd ? 24 : 12}>
                          ...
                          <span className="obj-properties">
                            ...
                            <span className="uptime-count">
                              {indicator.values.length > 0
                                ? (
                                    indicator.values[indicator.values.length - 1].value * 100
                                  ).toFixed(2)
                                : 0}
                              %
                            </span>{' '}
